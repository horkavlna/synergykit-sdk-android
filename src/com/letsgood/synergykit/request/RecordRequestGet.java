...
public class RecordRequestGet extends SynergyKITRequest{
	...
	protected Object doInBackground(Void... params) {
		...
		if(dataHolder.statusCode>= HttpStatus.SC_OK && dataHolder.statusCode < HttpStatus.SC_MULTIPLE_CHOICES){
			dataHolder.object = ResultObjectBuilder.buildBaseObject(dataHolder.statusCode, response.getBufferedReader(),config.getType());
		}
		else if(dataHolder.statusCode>=HttpStatus.SC_INTERNAL_SERVER_ERROR){
			dataHolder.statusCode = -1;				
		}
		else{
			dataHolder.errorObject = ResultObjectBuilder.buildErrorObject(dataHolder.statusCode, response.getBufferedReader());
		}		
